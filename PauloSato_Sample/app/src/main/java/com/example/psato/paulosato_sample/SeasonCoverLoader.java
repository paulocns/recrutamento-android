 ...
 */
public class SeasonCoverLoader extends AsyncTaskLoader<Bitmap> {

    static volatile private Bitmap mSeasonBanner = null;

    public SeasonCoverLoader(Context context) {
        super(context);
    }
    ...
    public Bitmap loadInBackground() {
        ...
        Bitmap result = null;
        try {
            URL url = new URL(Constants.URL_GET_SEASON_IMAGES);
    ...
    private String readCoverURLStream(InputStream in) {
        ...
        String imageSeasonCoverURL = null;
        try {
            JSONArray response = new JSONArray(result);
            int length = response.length();
            for (int i = 0; i < length; i++) {
                JSONObject season = response.getJSONObject(i);
                if (season.getString("number").equals(Constants.SEASONS)) {
                    JSONObject imageListObject = season.getJSONObject("images");
                    JSONObject posterImagesObject = imageListObject.getJSONObject("thumb");
                    imageSeasonCoverURL = posterImagesObject.getString("full");
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return imageSeasonCoverURL;
    }
    ...
    public void deliverResult(Bitmap imageCover) {
        ...
        Bitmap oldImagesCover = mSeasonBanner;
        mSeasonBanner = imageCover;
